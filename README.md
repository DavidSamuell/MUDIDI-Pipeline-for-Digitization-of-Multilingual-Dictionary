...
| `--stage1-agentic` / `--stage2-agentic` | off                   | Opt-in bounded verifier-rewriter loop after each enabled stage output; saves attempt audits under each page's `agentic/` directory. Tune with `--agentic-min-retry-confidence`, `--agentic-max-rewrite-delta-ratio`, and verifier-patch/concrete-gate ablation flags. |
...
| `--stage1-typography`                       | off         | Opt in to Stage 1 bold/italic annotation with `<b>`/`<i>` tags. By default Stage 1 emits plain OCR text.                                                                                                                                                                                          |
...
| `--stage1-agentic`                          | off         | After each Stage 1 page output, run a bounded verifier-rewriter loop before saving the final Stage 1 artifact.                                                                                                                                                                                    |
| `--stage2-agentic`                          | off         | After each Stage 2 MDF page output, run a bounded verifier-rewriter loop before saving the final MDF artifact.                                                                                                                                                                                     |
| `--agentic-max-iterations N`                | `2`         | Maximum rewrite attempts for each enabled agentic stage after the initial stage output.                                                                                                                                                                                                             |
| `--agentic-evaluator-model MODEL`           | stage model | Model for verifier calls; defaults to the current stage model.                                                                                                                                                                                                                                     |
| `--agentic-rewriter-model MODEL`            | stage model | Model for correction calls; defaults to the current stage model.                                                                                                                                                                                                                                   |
| `--agentic-reasoning {none,low,medium,high}` | `low`      | Reasoning effort for agentic verifier and rewriter calls.                                                                                                                                                                                                                                         |
| `--agentic-min-retry-confidence X`          | `0.55`      | Minimum verifier confidence required before a retry can rewrite. Lower-confidence retry findings are audited but do not trigger correction.                                                                                                                                                         |
| `--agentic-max-rewrite-delta-ratio X`       | `0.75`      | Reject a correction attempt when normalized text delta is larger than this ratio; rejected output is saved under the page's `agentic/` directory for audit.                                                                                                                                         |
| `--agentic-max-patches-per-attempt N`       | `16`        | Reject a verifier correction round that proposes more exact patches than this limit before it can rewrite the page.                                                                                                                                                                                 |
| `--no-agentic-verifier-patches`             | off         | Disable exact verifier-provided `current_text` → `expected_text` patches before falling back to the correction model.                                                                                                                                                                               |
| `--no-agentic-concrete-retry-gate`          | off         | Allow retry decisions without localized evidence. Mainly useful for ablations; the default gate is safer.                                                                                                                                                                                           |
...
- **Agentic verifier-rewriter loops** — `--stage1-agentic` and `--stage2-agentic` are opt-in, page-local loops. The normal stage output is attempt 0, a structured verifier decides `accept`/`retry`/`reject`, and correction attempts stop at `--agentic-max-iterations` or earlier if accepted, unchanged, rejected, stuck on the same issue, too low-confidence, too vague, too patch-heavy, or too different from the previous attempt. When the verifier provides an unambiguous `current_text` → `expected_text` edit, MUDIDI applies that patch before calling the correction model. Stage 1 is finalized before Stage 2 consumes it; Stage 2 is finalized before the MDF artifact is saved. See [`docs/agentic_verifier_rewriter_plan.md`](docs/agentic_verifier_rewriter_plan.md).
